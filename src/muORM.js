...
//
//Object with relationship will have a rel[] array filled up with muUID and / or array of muUID
// (depending on the cardinality of the relationship) for each child object relationship
//
// Object accessor will indirect to the instance of the object in the muStorage
//
//  ex. :   muORMTestRelationship11 -
//              .muORMTestObjectProperty
//              .rel[ ]
//                      -> 'muORMTestNumeric' : -muUID of the child object-
//              .muORMTestNumeric()-get- -> muStorage[ 'muORMTestNumeric' ][ -muUID of the child object- ]
//                      .rel[ ]
//                              -> 'muORMTestRelationship11' : -muUID of the parent object-
//                      .muORMTestRelationship11()-get- -> muStorage[ 'muORMTestRelationship11' ][ -muUID of the parent object- ]
//
//          muORMTestRelationshipNN -
//              .muORMTestObjectProperty
//              .rel[ ]
//                      -> muORMTestNNRelationShip[ ] : [ -muUID of the 1st object-, -muUID of the 2nd object-, ... ]
//              .muORMTestNNRelationShip()-get-[ ] -> muStorage[ 'muORMTestNumeric' ]
//                      .rel[ ]
//                              -> x
//
( function ( exports )
{
    exports.Schema                          = function( pathSchemas, defaultStorage, muSync )
    {
        this.pathSchemas        = '';
        this.defaultStorage     = defaultStorage ? defaultStorage : new mu.Storage.Driver( );
        this.objectList         = { };
        this.objectInstances    = { };
        this.muSync             = muSync ? muSync : null;
	...
	exports.Schema.prototype.linkSync = function( muSync )
	{
		this.muSync             = muSync ? muSync : null;

		if( !muSync.objectList ||
		    !muSync.objectInstances )
			muSync.linkStorageORMSchema( this );
	};
	...
	{
        for( var curObject in this.objectInstances )
        {
            if( curObject.relInstance )
                curObject.relInstance( );

            delete this.objectInstances[ curObject.muUID ];
        }
	};
    ...
    function loadDbSchemaFromCode( muORMSchema, jsonSnippet, jsObjectname )
    {
        ...
        with( scope )
        {
            this[ jsObjectname ] = function( optionalObjectOrMuUID )
                {   //this is the auto-generated dynamic constructor for the new prototype (javascript 'class')
                    ...
                    for( var prototypeAttributeName in objectPrototype )
                    {
                        ...
                        if( objectPrototype[ "constraint" ] != null &&
	                        ...
	                        (
		                        ...
		                        prototypeAttributeName,
		                        objectPrototype,
		                        optionalObjectOrMuUID && optionalObjectOrMuUID[ prototypeAttributeName ] ?
		                            optionalObjectOrMuUID[ prototypeAttributeName ] :
		                            objectPrototype[ "defaultValue" ][ prototypeAttributeName ]
	                        );

                        else if( optionalObjectOrMuUID && optionalObjectOrMuUID[ prototypeAttributeName ] )
                            this[ prototypeAttributeName ] = optionalObjectOrMuUID[ prototypeAttributeName ];
	                ...
	                if( typeof optionalObjectOrMuUID === "string" )
	                    this.muUID = optionalObjectOrMuUID;

                    var muUID = this.muUID;
	                objectPrototype[ "Storage" ].ownInstance
                    (
                        this,
                        jsObjectname,
                        function( objectOwned, objectPrototypeName )
                        {
                            delete muORMSchema.objectInstances[ objectPrototypeName ][ objectOwned.muUID ];
                        }
                    );

                    //if the storage is not the default, ownInstance will not add it up
                    // so we do
                    if( !muORMSchema.objectList[ jsObjectname ] )
                        muORMSchema.objectList[ jsObjectname ] = this.constructor;

                    if( !muORMSchema.objectInstances[ jsObjectname ] )
                        muORMSchema.objectInstances[ jsObjectname ] = { };

                    muORMSchema.objectInstances[ jsObjectname ][ this.muUID ] = this;

	                //if we're synced, we make sure that we get muSync properties pushed in the object
	                if( muORMSchema.muSync )
		                muORMSchema.muSync.addObjectInstanceToSync( this, jsObjectname );
                };
        }
        ...
        if( parsedObject[ 'muStorageType' ] )
            ...
            mu.warn( 'muORMSchema without a default storage !' );
        ...
        //we instantiate the object List both for the local storage AND our muORMSchema
        if( !objectGlobal.prototype[ "Storage" ].objectList[ jsObjectname ] )
        {
            objectGlobal.prototype[ "Storage" ].objectList[ jsObjectname ]        = objectGlobal;
            muORMSchema.objectList[ jsObjectname ] = objectGlobal;
        }

        if( !objectGlobal.prototype[ "Storage" ].objectInstances[ jsObjectname ] )
        {
            objectGlobal.prototype[ "Storage" ].objectInstances[ jsObjectname ]   = { };
            muORMSchema.objectInstances[ jsObjectname ] = { };
        }
