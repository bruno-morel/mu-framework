...
describe( "muStorage", function()
{
    ...
    it( "has a working persistent default storage (owning the object from the constructor)", function()
    {
        ...
        {
            mu.fs.unlinkSync( 'mockPrototypePersistent:1:mockProperty' );
            mu.fs.unlinkSync( 'mockPrototypePersistent:2:mockProperty' );
        }
    });
    ...
    it( "has a working session default storage (owning the object from the constructor)", function()
    {
        ...
        if( typeof window === "undefined" )
        {
            mu.fs.unlinkSync( '/tmp/mockPrototypeSession:1:mockProperty' );
            mu.fs.unlinkSync( '/tmp/mockPrototypeSession:2:mockProperty' );
        }
    });
