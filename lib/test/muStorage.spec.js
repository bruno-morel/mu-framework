...
describe( "muStorage", function()
{
    ...
    it( "has a working persistent default storage (owning the object from the constructor)", function()
    {
	    ...
	    if( !mu.runinbrowser )
	    {   //we only check for server side file creation
		    expect( fileExist( './mockPrototypePersistent:1:mockProperty' ) ).toBeTruthy( );
		    expect( fileExist( './mockPrototypePersistent:2:mockProperty' ) ).toBeTruthy( );
	    }
	    ...
	    if( !mu.runinbrowser )
	    {   //we only check for server-side file removal
		    expect( fileExist( './mockPrototypePersistent:1:mockProperty' ) ).toBeFalsy( );
	        expect( fileExist( './mockPrototypePersistent:2:mockProperty' ) ).toBeFalsy( );
	    }
    });
    ...
    it( "has a working session default storage (owning the object from the constructor)", function()
    {
	    ...
	    if( !mu.runinbrowser )
	    {   //we only check for server side file creation
		    expect( fileExist( '/tmp/mockPrototypeSession:1:mockProperty' ) ).toBeTruthy( );
		    expect( fileExist( '/tmp/mockPrototypeSession:2:mockProperty' ) ).toBeTruthy( );
	    }
	    ...
	    if( !mu.runinbrowser )
	    {   //we only check for server side file removal
		    expect( fileExist( '/tmp/mockPrototypeSession:1:mockProperty' ) ).toBeFalsy( );
	        expect( fileExist( '/tmp/mockPrototypeSession:2:mockProperty' ) ).toBeFalsy( );
	    }
    });
