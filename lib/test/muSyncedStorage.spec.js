    ...
    if( !mu.runinbrowser )
        it( "is returning 200 and the objects in HTML format when GETed for mockPrototypeAnonymous root '/mockPrototypeAnonymous'", function()
        {
            ...
            runs( function()
            {
	            ...
	            expect( responseValue.statusCode ).toEqual( 200 );
                expect( responseValue.contentType ).toEqual( 'text/html' );
                expect( responseValue.grabbedDatas ).toEqual( '<!DOCTYPE html><html><head><title>mockPrototypeAnonymous list</title></head><body><div><ul><li><a href="/v1/mockPrototypeAnonymous/' + currentMuUID1 + '">' + currentMuUID1 + '</a></li></ul><ul><li><a href="/v1/mockPrototypeAnonymous/' + currentMuUID2 + '">' + currentMuUID2 + '</a></li></ul></div></body></html>' );
			...
			runs( function()
			      {
				      ...
				      expect( splittedResponse[ 1 ] ).toEqual( '<ul>mockProperty<li>value injected from API</li></ul></div></body></html>' );
    ...
    if( !mu.runinbrowser )
        it( "is returning 200 and the object properties and values in HTML format when GETed with mockPrototypeAnonymous/[id]", function()
        {
            ...
            runs( function()
            {
	            ...
	            expect( responseValue.statusCode ).toEqual( 200 );
                expect( responseValue.contentType ).toEqual( 'text/html' );
                expect( responseValue.grabbedDatas ).toEqual( '<!DOCTYPE html><html><head><title>mockPrototypeAnonymous muUID=' + currentMuUID1 + ' properties</title></head><body><h1>muUID<li>' + currentMuUID1 + '</li></h1><div><ul>mockProperty<li>mockPropertyValue1</li></ul></div></body></html>' );
