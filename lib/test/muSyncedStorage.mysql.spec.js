...
describe( "muSync with MySQL Storage linked", function()
{
    ...
    muSyncMock = new mu.Sync.EndPoint( 'localhost', 8003, true );
			...
			runs( function()
			      {
				      ...
				      expect( responseValue.statusCode ).toEqual( 200 );
				      expect( responseValue.contentType ).toEqual( 'text/html' );
				      expect( responseValue.grabbedDatas ).toEqual( '<!DOCTYPE html><html><head><title>mockSyncedMySQLAnonymous list</title></head><body><div><ul><li><a href="/v1/mockSyncedMySQLAnonymous/' + currentMuUID1 + '">' + currentMuUID1 + '</a></li></ul><ul><li><a href="/v1/mockSyncedMySQLAnonymous/' + currentMuUID2 + '">' + currentMuUID2 + '</a></li></ul></div></body></html>' );
			      } );
		} );
			...
			runs( function()
			      {
				      muBrowser.get
				      (
					      'http://localhost:8003/v1/mockSyncedMySQLAnonymous/' + currentMuUID1,
					      function( statusCode, contentType, grabbedDatas )
					      {
						      responseValue.statusCode      = statusCode;
						      responseValue.contentType     = contentType;
			...
			runs( function()
			      {
				      expect( responseValue.statusCode ).toEqual( 200 );
				      expect( responseValue.contentType ).toEqual( 'text/html' );
				      expect( responseValue.grabbedDatas ).toEqual( '<!DOCTYPE html><html><head><title>mockSyncedMySQLAnonymous muUID=' + currentMuUID1 + ' properties</title></head><body><h1>muUID<li>' + currentMuUID1 + '</li></h1><div><ul>mockProperty<li>mockPropertyAnonymousValue1</li></ul></div></body></html>' );
			      } );
		} );
			...
			runs( function()
			      {
				      muBrowser.get
				      (
					      'http://localhost:8003/v1/mockSyncedMySQLAnonymous/' + currentMuUID1 + '.json',
					      function( statusCode, contentType, grabbedDatas )
					      {
						      responseValue.statusCode      = statusCode;
						      responseValue.contentType     = contentType;
