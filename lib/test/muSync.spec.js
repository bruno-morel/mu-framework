...
describe( "muSync", function()
{
    ...
    if( !mu.runinbrowser )
        it( "is returning 503 when root is browsable but there's no mu object", function()
        {
            var asyncTick = false;
            var responseValue = { statusCode : 0, contentType : '', grabbedDatas : '' };
    ...
    if( !mu.runinbrowser )
        it( "is returning 503 when root is browsable but there's no mu object in JSON format", function()
        {
            var asyncTick = false;
            var responseValue = { statusCode : 0, contentType : '', grabbedDatas : '' };
