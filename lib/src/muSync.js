...
( function ( exports )
{
    exports.EndPoint                = function( port, browsable, muORMSchema, version, keyPath, certPath )
    {
        ...
        if( request.url.lastIndexOf( '/v' + muSyncObject.version ) == -1 )
        {
            ...
            response.data       =   renderObject
                                    (
	                                    ...
	                                    '/',
	                                    generateErrorMessage( 1, '"' + request.url + '"' ),
	                                    inJson
                                    );
        }
        else
    ...
    function extractLeafOrRoot( muSyncObject, request, response, inJson )
    {
        ...
        if( isRoot &&
	        ...
	        request.method == "GET" )
            getObjectsList( muSyncObject, response, inJson );

        else if( !isRoot &&
                 muSyncObject.objectList != null )
	        parseRequest( muSyncObject, request, response, inJson );
		else if( isRoot &&
		         !muSyncObject.browsable )
        {   //non-browsable root with GET attempt
	        ...
	        response.data       = renderObject
		        (
			        muSyncObject.APIName != null ? muSyncObject.APIName + ' API' : 'Unknown API',
			        request.url,
			        generateErrorMessage( 5, '"' + request.url + '"' ),
			        inJson
		        );
        }
	    else
        {   //unauthorized call / method
			response.code       = 504;
	        response.data       = renderObject
		        (
			        muSyncObject.APIName != null ? muSyncObject.APIName + ' API' : 'Unknown API',
    ...
    function parseRequest( muSyncObject, request, response, inJson )
    {
        ...
        {   //we only recognize .json and .html / htm request for now
            ...
            response.data       = renderObject
                (
                    leafObject + ' ',
                    request.url,
                    generateErrorMessage( 5, leafObject.substring( leafObject.indexOf( '.' ) ) ),
                    inJson
                );
            return;
        }
        ...
        if( isNaN( parseFloat( objectNumeric ) ) ||
            ...
            parseObjectListRequest( muSyncObject, leafObject, request, response, inJson );
        ...
        {   //we got a numerical value => this is an object description request
	        ...
	        if( leafObject == ( 'v' + muSyncObject.version ) )
	        {
	            ...
	            parseObjectInstanceRequest( muSyncObject, leafObject, objectNumeric, request, response, inJson );
        }
    }
    ...
    var errorCodeMessage =
    {
        ...
        2 : ' schema does not exist or is empty',
