...
( function ( exports )
{
    ...
    exports.prepare         = function( storagePathOrURI, userLogin, userPassword, params )
    {
        ...
        if( mySQLDrv.relInstance == null )
            mySQLDrv.relInstance  = mysqlDrvItemRelease;
        ...
        if( mySQLDrv.Storages[ storageURL ] == null )
        {
			...
			//this will allow a locking mechanism to avoid unwanted async disconnect
	        ...
	        if( mySQLDrv.keepAlive == null )
		        mySQLDrv.Storages[ storageURL ].keepAlive = function( ){ mySQLDrv.Storages[ storageURL ].keepAliveFlag = true; };

	        if( mySQLDrv.freeToDie == null )
		        mySQLDrv.Storages[ storageURL ].freeToDie = function( ){ mySQLDrv.Storages[ storageURL ].keepAliveFlag = false; };
	...
	function mysqlDrvDisconnect( storageContent )
	{
		...
		for( var transactionPending in storageContent.objectOrPropertyLastTransactionID )
		{
			if( storageContent.objectOrPropertyLastTransactionID[ transactionPending ] != 0 )
			{
	...
	function mysqlDrvItemRelease( objectStorageObject, prototypeURN, objectToOwn )
	{
		mysqlDrvGetPoolConnectionAndCall( objectStorageObject, prototypeURN + ':' + objectToOwn.muUID, mysqlDrvItemDel );
	}
    ...
    function mysqlDrvPrototypeAdd( objectStorageObject, objectURN, connectionFromPool, transactionID )
    {
        ...
        var createQueryString= 'CREATE TABLE IF NOT EXISTS ' + mu.mysql.escapeId( prototypeName ) +
                                    '( muUID INTEGER UNSIGNED NOT NULL, PRIMARY KEY( muUID ), ';
        var propertyType    = '';
    ...
    function mysqlDrvItemDel( objectStorageObject, objectURN, connectionFromPool, transactionID )
    {
	    var storageContent = objectStorageObject.driver.Storages[ objectStorageObject.storagePathOrURI ];
	    var objectToSync = objectURN.split( ':' );
	    var prototypeName = objectToSync[ 0 ];
